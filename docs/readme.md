...
> # ⛔ 本文含已被推翻的结论 —— 阅读前必看
>
> **状态**：部分作废（第 1/2/6 章的“机械臂型号未定”和整个第 7 章的控制器/运动结论已作废；
> 第 3/4/5/8 章的坐标约定、标定数据、目录规范和相机边界仍然有效）
> **推翻日期**：2026-08-22（依据：控制器原始日志 + 出厂配置备份）
> **权威文档**：[纳博特C1102-现场真相-必读.md](纳博特C1102-现场真相-必读.md)
> **原始证据**：[根因证据-控制器日志摘录.txt](现场备份-20260822/根因证据-控制器日志摘录.txt)
>
> 本文写于到场前的准备阶段，第 7 章整章建立在“机械臂随机分配、型号与运动协议均未知”
> 这个前提上。实机为 **MOKA 藦卡 MR07S-930 + 纳博特 Inexbot C1102，固件 21.05.23-4.1.1，
> IP 192.168.1.200**，运动协议已经在现场逐字节确证：错不在“协议推不出来”，而在于
> 出厂配置 `global.json` 的 `RemoteIO[0].posReset` 里 `safeEnable=true` 但逐轴
> `deviation=null`，导致**远程模式下的复位点安全闸门永远判失败并顺带把伺服下电**。
> 保留本文是为了让后来的 agent/人知道**哪条路已经试过并且是错的**，
> 不要把第 7 章的任何操作建议（尤其是“Modbus-GP 保底”“远程模式运行”）当作可执行方案。
>
> | 本文的旧结论 | 现已确证的事实 |
> |---|---|
> | 机械臂随机分配、型号未定，不应猜测端口和报文 | 型号已确定：MOKA MR07S-930 + Inexbot C1102，固件 21.05.23-4.1.1，IP 192.168.1.200 |
> | MOVJ/MOVL 不能从手册推导，必须拿到官方 SDK 才能接入 | 0x4501/0x4502 已实测可用，**必须带 acc 和 dec**；解析顺序 robot→vel→acc→dec→coord→pos[7]，补齐后 ±5mm/-3mm 精确到位。22.07 公开文档漏了 acc/dec |
> | 私有运动 TCP 调不通时，用 Modbus-GP 作业文件保底 | 运动通道本身没问题，问题是**运行模式**：0x3003/0x3007/0x3002/0x4501/0x4502 走同一入口 `startRobotJobTask(..., safepos=1, call=moveToPos)`，远程模式 15 次全被闸门拒绝，示教模式 40+ 次全成功 |
> | 掉伺服是因为发了不可达/无效指令 | 每被闸门拒绝一次，控制器就执行 stop → JobClear → 脉冲使能=0 → Deadan_End → PowerOff，**拒绝一次就下电一次**；机器人精确停在复位点、逐轴只差 1e-5° 也照拒 |
> | 远程模式是自动运行的正常前提（示教器拔出即进入） | 本机远程模式是**最危险**的模式；示教模式下 0x2311 上使能后 status=3 会一直保持，也不需要按 deadman |
> | 需要手动按住示教器 deadman 才能动 | 不需要；0x2301 是示教器按键通道，上位机发无效（恒回 deadman:0） |
> | 固件版本待确认（22.07/24.03） | 实机固件 21.05.23-4.1.1；不要照抄 22.07 公开文档的字段表 |

更新时间：2026-08-22（加入现场真相横幅与逐条批注；正文主体最后更新于 2026-08-19）

（原文此处残留 git 冲突标记 `<<<<<<< HEAD` / `>>>>>>> 9505fd07`，两侧分别写着
2026-08-19 和 2026-08-17 两个更新时间，已在本次改造中合并为上面一行。）
...
> ❌ **已推翻（表格最后一行）**：型号与通信接口都已确定并实测——MOKA 藦卡 MR07S-930 +
> 纳博特 Inexbot C1102，固件 21.05.23-4.1.1，IP 192.168.1.200；0x4501/0x4502 直角/关节
> 运动在**示教模式**下 40+ 次全部成功到位（±5mm/-3mm）。当前状态应读作
> “**已接入，但只能在示教模式下运动**”：远程模式被 `posReset` 安全闸门拒绝，
> 且每拒绝一次伺服下电一次。详见 [纳博特C1102-现场真相-必读.md](纳博特C1102-现场真相-必读.md)。
...
`docs/` 中保存了 OAK-D-Pro 产品手册、两类候选机械臂资料和考核说明。

> ~~机械臂为随机分配，确认实际型号并取得官方 SDK 或通信协议前，不应根据操作手册猜测控制端口和报文。~~
> ❌ **已推翻**：型号已确定为 MOKA 藦卡 MR07S-930 + 纳博特 Inexbot C1102（固件
> 21.05.23-4.1.1，IP 192.168.1.200），控制端口与报文已逐字节实测确证，不再是猜测。
> 需要注意的反而是：**不要照抄 open.inexbot.com 的 22.07 公开文档**——它在 0x4501/0x4502
> 的字段表里漏了 `acc` 和 `dec`，照抄会得到“参数错误”并误导出“本固件不支持 0x45xx”的错误结论。
...
3. ~~确认比赛机械臂型号，接入只读关节/TCP 状态，明确数据是法兰还是 TCP；在完成厂商
   ...
   ❌ **前半句已完成/已推翻**：型号已确定（MR07S-930 + C1102），只读状态已接入。
   仍然有效的部分：法兰/TCP 的区分、`dry_run` 门禁。**新增的硬性前置**：
   运动前必须 0x2002 查状态、`!=3` 就 0x2311 上使能，且**每段运动都要重查**
   （远程模式下每被拒绝一次就掉一次电）；运动后必须等 0x3D03 `status=2`
   才算真的动了，“位姿不变”绝不能当成功。
...
> ⛔ **整章降级为历史存档 + 反例**（2026-08-22）
>
> 本章写于无实物阶段，通篇的隐含前提是“运动协议未知、需要找一条能用的运动通道”。
> 现场结论是：**运动通道从来不是瓶颈，运行模式和出厂配置才是**。
> 章内仍然正确、可继续使用的是：7.2 七点法 TCP 标定、7.3 三点法用户坐标、
> 7.4 手眼与运行时定位策略、7.5 形态值定义。**已作废**的是所有关于
> “MOVJ/MOVL 推不出来”“Modbus-GP 保底”“远程模式是正常运行前提”的论断，逐条见下方批注。
>
> 上机前请先读 [纳博特C1102-现场真相-必读.md](纳博特C1102-现场真相-必读.md)。
...
[`纳博特通讯协议.md`](纳博特通讯协议.md)（帧格式、6001 端口 MOVJ/MOVL 命令字、
7000 端口状态查询，CRC 已验证）。真实通信适配器可据此实现；

> ~~现场仍需确认固件版本（22.07/24.03）、6001 端口直连行为和坐标单位，不能凭空猜测未定义的字段。~~
> ❌ **已确认，不再是待办**（证据：`现场备份-20260822/SystemBackup08-22-22-13/`）：
> - 固件版本 **21.05.23-4.1.1**，不是 22.07 也不是 24.03；
> - 0x4501（直角）/0x4502（关节）在本固件**可用**，但**必须带 acc 和 dec**，
>   解析顺序为 `robot → vel → acc → dec → coord → pos[7]`。22.07 公开文档漏了 acc/dec 两个字段，
>   照抄会回“参数错误”。补齐后实测 ±5mm/-3mm 精确到位；
> - 坐标单位：`realPosACS` 本来就是**度**；`realPosUCS`/`realPosPCS` 的 A/B/C 是**弧度**
>   （旧说法“realPosACS 需要 ×57.29578”是错的）；
...
> ⚠️ **本机补充（2026-08-22 实测）**：这份只读采集清单本身没错，但少了最关键的一项——
> **出厂配置 `global.json` 的 `RemoteIO[0].posReset`**。本机是 `safeEnable=true` 而逐轴
> `deviation=null`，闸门判据坏掉，**远程模式下永远判“机器人 1 不在安全位置附近”**，
> 机器人精确停在复位点、逐轴只差 1e-5° 也照拒；拒绝后控制器执行
> stop → JobClear → 脉冲使能=0 → Deadan_End → PowerOff，即**每拒绝一次就把伺服下电一次**。
> 所以到场第一件事是导出 `global.json` 看这两个字段，再决定用哪种模式。
> 另外：**示教模式**下 0x2311 上使能后 `status=3` 会一直保持，不需要按 deadman；
> 0x2301 deadman 是示教器按键通道，上位机发无效（恒回 `deadman:0`）。
> 还有：复位点判定 3x101=1 与 ioControl 的闸门是**两条不同代码路径**，
> 3x101=1 时闸门照样拒绝，不能拿它当“闸门能过”的证据。
...
> ⚠️ **本机补充：这里的“弧度”正是摔臂的直接原因，必须逐条对齐单位**（2026-08-22）
> - `coord=3` 是**用户坐标系**（本机实测），不是旧文档说的“3=工具 / 2=用户”；
>   整条流水线以**用户坐标系 1** 为准。
> - `realPosACS`（关节角）本来就是**度**，不要再乘 57.29578；
>   `realPosUCS`/`realPosPCS` 的 A/B/C 才是**弧度**。
> - `teleport_panel` 曾有一个“A/B/C 用弧度”复选框：回读永远往输入框里填**度**，
>   勾选后 `_pending()` 把度当弧度发出。2026-08-22 控制器日志 16:58:19 收到
>   A/B/C=(1.9406, 1.1961, 2.2181) rad，正是回读度数 (177.8697, 13.7625, -179.9943)
>   被当弧度后折叠的结果（反解残差 ≤1.04e-4 度 = 编码器抖动）。该姿态与真实姿态相差
>   **119.6°**，而 XYZ 只走 30 mm 直线；6 秒后六轴同时 0F15 故障 → 控制器 PowerOff →
>   **机械臂坠落**，此后持续 `robot_sync=0, n=6`，每次上电立刻 `Can_powerdown`。
>   **该复选框已永久删除，不要再加回来。**
> - 因此下发前必须过三道闸门：位移 ≤400 mm、`|A/B/C| ≤7 rad`（专门挡“度当弧度”）、
>   姿态变化 ≤20°。

建议保存为控制器 `User1`，同时把 `T_base_user1` 读回并写入
...
> ❌ **已部分推翻（2026-08-22 实测）**：厂商单点运动语义已经验收——
> 0x4501（直角）/0x4502（关节）在**示教模式**下 40+ 次全部成功到位（±5mm/-3mm）。
> 补充三条落地要求，上面“每个点带完整坐标系、Tool/User ID 和形态字段”还要再加：
> 1. **必须带 acc 和 dec**，解析顺序 `robot → vel → acc → dec → coord → pos[7]`；漏掉即“参数错误”。
> 2. 运动后**必须等 0x3D03 `status=2`** 才算真的动了（`_await_motion_ack`）；
>    “位姿不变”绝不能当成功——远程模式下正是“接受了但不动”，实际是被闸门拒绝并掉了电。
> 3. 报警帧的真实文本在 0x2B03 的 **`data`** 键，**不是 `message`** 键；
>    读错键会看到空报警而误判“无故障”。
>
> 仍然待验收的只剩：**多点点列、blend 过渡**这两项。

### 7.5.1 Modbus-GP 运动保底

> ⛔ **本小节整节作废，仅作反例保留**（2026-08-22）
>
> 这一节的前提是“私有运动 TCP 可能调不通，所以需要一条 Modbus 作业文件的后路”。
> 这个前提是错的，而且它衍生出的旧结论
> “~~0x3003 GO_POSITION 被接受但永远不动，运动只能靠 Modbus 作业文件(45/19/29/61/71)~~”
> 已被控制器原始日志直接推翻：
>
> - 0x3003 / 0x3007 / 0x3002 / 0x4501 / 0x4502 在控制器内部走**同一个入口**
>   `startRobotJobTask(..., safepos=1, call=moveToPos)`；
> - 它们在**远程模式**下被 `posReset` 复位点安全闸门拒绝（15 次全拒），
>   在**示教模式**下正常执行（40+ 次全成功，零例外）；
> - 所以“换一条运动通道”从头到尾都解决不了问题——**Modbus-GP 作业文件走的是同一个闸门**，
>   换通道只是换一种被拒绝的方式，而每拒绝一次伺服就下电一次。
>
> 特别警告：旧文档里“**0x3007 回复位/回零 已验证通过、是零风险备选方案**”是最危险的一条。
> 它是每条抓取序列的第一句，在远程模式下**必然掉使能**。
>
> 正确做法是修 `global.json` 的 `RemoteIO[0].posReset`（`deviation=null` 是坏值），
> 或在示教模式下运行，而不是另找运动通道。下面的原文保留，不要照做。
...
> ⚠️ **本机重大警告**：手册这段描述本身没错，但它暗示的“远程模式 = 自动运行的正常前提”
> 在本机是**危险的**。本机远程模式下每一条运动指令都会被 `posReset` 闸门拒绝，
> 而每次拒绝都伴随一次伺服下电（stop → JobClear → 脉冲使能=0 → Deadan_End → PowerOff）。
> **示教模式**下 0x2311 上使能后 `status=3` 会一直保持，运动 40+ 次零例外，
> 且**不需要按 deadman**。在 `global.json` 的 `RemoteIO[0].posReset.deviation` 修好之前，
> 不要把“拔示教器进远程模式”当成上机流程的一部分。
...
> ⚠️ **本机补充（2026-08-22）**：夹爪走 **0x3601**，是**另一条码路**，
> **不受使能状态与 `posReset` 安全闸门影响**。因此现场调试时反复出现的
> “夹爪能动，所以控制器是好的、机器人应该也能动”是**错误推理**——
> 夹爪能动**推不出**机器人能动，两者的失败模式完全独立。
> 这条与上一段是同一个道理：不要用一个通道的成功去替代另一个通道的验证。

基础通信代码和现场确认表见
[`competition_pipeline/controller_protocol.md`](../competition_pipeline/controller_protocol.md)。
当前实现标准 Modbus-TCP 的帧、超时、异常和配置化 IO 访问；

> ~~由于本手册没有公开运动 socket 报文，MOVJ/MOVL 仍不能从该手册推导，必须拿到官方 SDK/通信协议后接入。~~
> ❌ **已推翻**：运动报文已经逐字节确证并实测到位，不需要额外的官方 SDK。
> 用 0x4501（直角）/0x4502（关节），**必须带 acc 和 dec**，
> 解析顺序 `robot → vel → acc → dec → coord → pos[7]`，`coord=3` 为用户坐标系。
> 真正的阻塞点从来不是报文，而是远程模式下的 `posReset` 安全闸门。
...
> ⚠️ **读报警文本的正确位置**：下一段说的“控制器报错文本”，其真实内容在
> **0x2B03 回帧的 `data` 键**，不是 `message` 键。读错键会得到空字符串，
> 于是把“有报警”误判成“无报警”，进而把被闸门拒绝的运动误判成成功。
...
> 📌 **历史存档**：这里的“明天”指 2026-08-21，该准备日已过，现场也已完成。
> 表格里的产出项仍可作为交接物清单参考，但“不填写猜测的控制器 IP、端口、寄存器”
> 一句已经过期：IP 为 192.168.1.200，端口与报文均已实测确证。
> 末尾“到场后第一小时只做只读采集、第二小时才开始夹笔七点法”的时间安排同样已经作废。
