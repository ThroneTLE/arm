...
界面上还有一组与视觉流程无关的手动控件，用来排查和临时干预：

- **夹爪【张开】【闭合】【读状态】** —— 走 `0x3601` DOUT，**不经过运动闸门，
  也不受【空跑】限制**。上电后先单独试夹爪，能在完全不动机械臂的前提下证明
  连接是通的。现场实测映射 `(DOUT15, DOUT16) = (1,0)` 闭合 / `(0,1)` 张开；
  读回 `(0,0)` 或 `(1,1)` 一律报"状态不可判定"，不会替你猜是开是合。
  回读失败时说的是"已下发但未确认"，**不会**报成功。
- **【读取当前 TCP】** —— 只读，读完自动填进传送框。用户坐标系1 的读数就是
  工具末梢（本机工具手1 已标到夹爪尖端）。
- **【传送到该点】** —— 只动 XYZ、姿态保持当前姿态；走全部闸门，受【空跑】限制。
- **速度 / 到位停顿 / 气阀停顿** —— 速度就是 MOVL 帧里 `vel` 的值（mm/s，
  中间不再缩放）；气阀停顿是合爪指令下发后的等待，少了会抓空。
